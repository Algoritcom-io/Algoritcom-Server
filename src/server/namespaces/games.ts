...
const MAX_PLAYERS = Number(process.env.MAX_GAMES_PLAYERS) || 8;

const games = io.of("/games");
const rooms = io.of("/games").adapter.rooms;
...
games.on("connection", (socket: any) => {
  try {
    const player: Player = {
    ...
    socket.on("game-join", (roomName: string) => {
      if (!roomName) {
      ...
      if (
        ...
        const playersInRoom = games.adapter.rooms.get(roomName);
        if (playersInRoom) {
    ...
    socket.on("game-leave", (roomName: string) => {
      if (roomName) {
        let player = players.get(socket.id);
